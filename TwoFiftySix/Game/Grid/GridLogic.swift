...
struct GridLogic: GridLogicType {
    ...
    init(grid: Grid) {
        ...
        func traversals(forMoveDirection direction: MoveDirection) -> [Traversal] {
            var starts = [Slot]()
            switch direction {
            case .up:
                for column in 0..<4 {
    ...
    /// move have _already_ been encountered and moved.
    /// - Parameters:
    ///   - traversal: A Traversal..
    ///   - slot: The slot (which must be in the `traversal` array) where we encountered
    ///   the tile we are considering moving.
    ...
    private func furthestEmpty(in traversal: Traversal, from slot: Slot) -> Slot? {
        var result: Slot? = nil
        var current = slot + traversal.direction.vector
        while traversal.array.contains(current) {
            if grid[current] == nil {
                result = current
            }
            current = current + traversal.direction.vector
        }
        return result
    }
    ...
    func closeUp(traversal: Traversal) {
        for slot in traversal.array {
            if let tile = grid[slot] {
                if let empty = furthestEmpty(in: traversal, from: slot) {
                    grid[empty] = tile
                    grid[slot] = nil
                }
            }
    ...
    /// After `closeUp(traversal:)` is initially applied to a traversal, if any tiles
    ...
    ///   - traversal: The Traversal.
    func merge(traversal: Traversal) {
        for pair in traversal.array.windows(ofCount: 2) {
    ...
    /// This is the Really Tricky Part. After a call to `userMoved(direction:)`, the grid may have
    /// changed. We need to enact those same changes in the visible interface. In order to do so, we
    /// need to know what those changes are! This method returns an Assessment, which is simply a
    /// description of the changes that took place. I suppose we could have tracked the changes as
    /// we performed them, but we didn't. So how do we know what they were? This is why a Tile has
    /// `column` and `row` properties and an `absorbed` property. If a tile's `column` and/or `row`
    /// do not match the slot where it is now, it was moved from the `column` and `row` of its
    /// properties; so report the movement, and rectify its `column` and `row`. If a tile has an
    /// `absorbed` tile, that tile was merged into this one; so report that, and nilify its
    /// `absorbed` property.
    ///
...
fileprivate extension Traversal {
    /// The only "public" initializer for a traversal, and even that is available only
    /// by the GridLogic object.
    /// - Parameters:
    ///   - slots: The array of slots.
    ///   - direction: The move direction.
    init(_ slots: [Slot], direction: MoveDirection) {
        self.array = slots
